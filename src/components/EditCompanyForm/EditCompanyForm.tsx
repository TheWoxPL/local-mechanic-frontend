...
import { UpdateCompanyDTO } from 'src/shared/dtos/update-company.dto';
...
export const EditCompanyForm = ({
  ...
  useEffect(() => {
    if (companyData) {
      // Set form default values from company data
      reset({
        companyName: companyData.companyName,
        description: companyData.description || '',
        address: companyData.address || '',
        foundDate: companyData.foundDate
          ? new Date(companyData.foundDate)
          : new Date(),
        workingHoursFrom: companyData.workingHours?.from || 8,
        workingHoursTo: companyData.workingHours?.to || 16,
        phoneNumber: companyData.phoneNumber || '',
      });

      // Set image preview if there's an image URL
      if (companyData.imageUrl) {
        setImagePreview(companyData.imageUrl);
      }
    }
  }, [companyData, reset]);
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      const updateData = new UpdateCompanyDTO();
      updateData.companyName = data.companyName;
      updateData.description = data.description || '';
      updateData.foundDate = data.foundDate || new Date();
      ...
      const updatedCompany: CompanyDTO = await ApiUtils.companies.updateCompany(
        companyId,
        updateData
      );
      ...
      if (companyFile) {
        ...
        formDataForFile.append('companyId', companyId);
        await ApiUtils.companies.uploadCompanyImage(formDataForFile);

        const refreshedData =
          await ApiUtils.companies.getCompanyById(companyId);
  ...
  return (
    <div className={styles.container}>
      <div className={styles.form}>
        ...
        <h2 className={styles.formTitle}>Edit data</h2>
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className={styles.row}>
            ...
            <input
              ...
              id="foundDate"
              defaultValue={
                companyData.foundDate
                  ? new Date(companyData.foundDate).toISOString().split('T')[0]
